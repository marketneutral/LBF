...
parser.add_argument(
    ...
    default="fix",
    type=str,
    choices=["fix", "ma-fix", "reinforce", "mapo"],
...
generate = GenerateNode(
    hidden_size=hidden_size, op_nums=5, embedding_size=embedding_size
)
...
for epoch in range(n_epochs):
    ...
    pos = 0
    epo_iteration = 0
    for idx in tqdm(range(len(input_lengths))):  # batch

        if idx < 2 and epoch == 0:
            mask_flag = True
        buffer_batches_train = buffer_batches[pos : pos + len(input_lengths[idx])]
        buffer_batches_train_exp = buffer_batches_exp[
            pos : pos + len(input_lengths[idx])
        ]
    ...
    if epoch % 5 == 0 or epoch > n_epochs - 5:
        ...
        for k in range(len(test_pairs)):
            ...
            for i in range(0, len(test_results)):
                ...
                val_ac, test_exp = compute_prefix_tree_result(
                    test_res, test_batch[4], output_lang, test_batch[2]
                )
