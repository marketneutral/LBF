...
def train_tree(
    ...
    for gen_length in gen_lengths:
        ...
        for t in range(max_target_length):
                ...
                if t >= target_length[i]:
                    ...
                    if generated_ops[i] >= (target_length[i] - 1) / 2:
                        op2[
                            ...
                            -1e10
                        )  # number of ops cannot be greater than (target_length-1)/2
                    if (
                        generated_nums[i] == generated_ops[i]
                        and t < target_length[i] - 1
                    ):
                        num_score2[
                            i, :
                        ] = (
                            -1e10
                        )  # except the last postion, number of nums cannot be greater than number of ops
                    if t == 0 and target_length[i] > 2:
                        num_score2[
                            i, :
                        ] = (
                            -1e10
                        )  # first cannot be number unless target_length equals to 1
                    ...
                    if mask_flag:
                        num_score2[i][
                            ...
                            -1e10
                        )  # for the first iterations, do not generate 1 and 3.14
            ...
            out_score = nn.functional.log_softmax(
                torch.cat((op2, num_score2), dim=1), dim=1
            )
            ...
            topi_t, generate_input = generate_tree_input(
                topi.tolist(), outputs, num_start
            )
            ...
            for idx, l, r, node_stack, i, o in zip(
                ...
                if i < num_start:
                    ...
                    current_num = current_nums_embeddings[idx, i - num_start].unsqueeze(
                        0
                    )
                    ...
                    if eval(generate_infix) == eval(num_ans[idx]):
                        ...
                        if model == "mapo":
                            if not exp in buffer_batch_new[idx]:
                                buffer_batch_new[idx].append(
                                    exp.item()[: target_length[idx]]
                                )
    ...
    for j in range(num_iteration):
        if not j * mapo_batch_size + mapo_batch_size - 1 < len(fix_target_list):
            mapo_batch_size = len(fix_target_list) - (
                (j - 1) * mapo_batch_size + mapo_batch_size
            )
        target_length_mapo = fix_target_length[
            j * mapo_batch_size : (j * mapo_batch_size + mapo_batch_size)
        ]
        idx_list = fix_index[
            j * mapo_batch_size : (j * mapo_batch_size + mapo_batch_size)
        ]
        target_list = fix_target_list[
            j * mapo_batch_size : (j * mapo_batch_size + mapo_batch_size)
        ]
        input_length_mapo = []
        num_pos_mapo = []
        ...
        input_var_mapo = torch.zeros(
            (max(input_length_mapo), mapo_batch_size), dtype=torch.long
        )
        target = torch.zeros(
            (mapo_batch_size, max(target_length_mapo)), dtype=torch.long
        )
        ...
        num_size = max(copy_num_len)
        all_nums_encoder_outputs = get_all_number_encoder_outputs(
            encoder_outputs, num_pos_mapo, batch_size, num_size, encoder.hidden_size
        )
        num_start = output_lang.num_start
        embeddings_stacks = [[] for _ in range(batch_size)]
        ...
        for t in range(max_target_length):
            ...
            target_t, generate_input = generate_tree_input(
                target[t].tolist(), outputs, num_start
            )
            target[t] = target_t
            if USE_CUDA:
                generate_input = generate_input.cuda()
        ...
        loss1 = masked_cross_entropy(
            all_node_outputs, target.float(), target_length_mapo
        )
        # loss = loss1
        loss1.backward()
        loss += loss1
    ...
    return (
        ...
        buffer_batch_new,
        num_iteration,
        buffer_batch_new_exp,
    )  # , loss_0.item(), loss_1.item()
    # return 0
